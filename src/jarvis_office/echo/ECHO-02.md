...
Other prefill choices: 20, 40, 60, 80, 120, 140 ms. Stop the dedicated Echo listener before qualification; the harness owns/stops its temporary listener and Android process. Never stop the main Jarvis service. `--phase micro_replay --seconds 6` requires the owner ready to say “Jarvis, test du microphone Echo Show.” and forbids `--capture-file`. No STT, TTS or cloud API is involved. Mutable capture and replay buffers are cleared on success/failure; no audio file is needed.
...
Full server validation: 120 unit tests, Ruff check/format, mypy; no real models or cloud calls. Acoustic answers, prescribed real-microphone replay, live addressed turns, passive turquoise test, clear-after-live-context, no self-trigger and 30-minute stability remain required. Detailed sanitized matrix and APK identity are in the Android repository's `ECHO-02.md`; raw reports and credentials stay outside Git.
