...
`RemoteEchoEgress` adapts streamed Qwen PCM to PCM16 mono 48 kHz and sends 20 ms binary packets. All linguistic segments in one assistant turn share one stream. Source chunks, pipe, ingress queue and fractional downlink frame are bounded; there is no WAV, replay or whole-response accumulation. Playback acknowledgements, session identity, uplink generation and turn identity are checked before delivery. Segment confirmation is conservative: only a fully drained remote response confirms its segments.
