...
- Boucle vocale : départ en pause, capture fermée avant STT/réponse, réarmement borné après lecture et délai acoustique. Les fenêtres Echo se renouvellent tant que le mode autorisé reste actif, sans quota de tours. L'inactivité ne coupe pas le micro. OFF explicite, clear/interrupt ou perte du chemin réseau arrêtent la capture ; une reconnexion du même appareil reprend le dernier mode choisi. Le smoke diagnostic reste borné à un tour. Annuler/Pause ne réarment pas ; Effacer invalide session et événements. En Conversation (ACTIVE), toute transcription acceptée est une demande. En Commandes (COMMAND), Jarvis est optionnel ; le dispatcher TV/musique/streaming répond par un son OK ou erreur, sans phrase TTS ni historique. En écoute contextuelle et en CLI locale, jamais de transcription sans adresse Jarvis affichée, journalisée ou envoyée.
