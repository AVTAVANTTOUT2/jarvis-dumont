...
class DashboardTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.state = {"schema_version": 1, "server_epoch": "epoch", "event_id": 1,
                      "devices": [], "budget": {"diagnostic": {"used": 9, "limit": 10}}}
        self.commands = []
        ...
        async def context(device):
            return {"entries": [], "limits": {"seconds": 1800, "chars": 20000, "utterances": 100},
                    "next_turn_max_chars": 3500}

        self.server = DashboardServer(self.store, snapshot=snapshot, command=command,
                                      context=context, port=0)
    ...
    async def post(self, path, data, **headers):
        return await self.client.post(self.server.url + path, json=data,
                                      headers={"Origin": self.server.url,
                                               "X-CSRF-Token": self.csrf, **headers})

    async def test_owner_auth_origin_host_and_echo_token_separation(self):
        async with aiohttp.ClientSession() as other:
            async with other.get(self.server.url + "/api/state",
                                 headers={"Sec-Fetch-Site": "same-origin"}) as response:
    ...
    async def test_settings_csrf_validation_and_nominal_budget_gate(self):
        response = await self.post("/api/settings", {"history_enabled": True},
                                   **{"X-CSRF-Token": "bad"})
        ...
        response = await self.post("/api/command", {"device_id": "echo", "action": "preview_voice",
                                                   "command_id": "local-preview"})
        self.assertEqual((await response.json())["status"], "applied")
        self.assertEqual((await self.store.settings())["budget"]["used"], 0)
    ...
    async def test_data_routes_scope_confirmation_and_backup(self):
        ...
        async with self.client.get(self.server.url + "/api/data/rows",
                                   params={"table": "turns", "filters": filters}) as response:
            self.assertEqual(response.status, 400)
        ...
        response = await self.post("/api/command", {"device_id": "echo", "action": "clear_context",
                                                   "command_id": "clear"})
        self.assertEqual(response.status, 400)
        await response.release()
    ...
    async def test_lan_configuration_and_request_size_are_rejected(self):
        with self.assertRaisesRegex(ValueError, "DASHBOARD_LOOPBACK_ONLY"):
            DashboardServer(self.store, snapshot=self.server.snapshot_callback,
