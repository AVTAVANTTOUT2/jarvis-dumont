...
| DeepSeek API, `deepseek-flash` (DeepSeek-V4.1-Flash) | Service distant ; aucun octroi de licence de poids déduit du contrat technique. [Documentation officielle](https://api-docs.deepseek.com/), [Chat Completions](https://api-docs.deepseek.com/api/create-chat-completion). Conditions contractuelles du compte non auditées ici. | Requête nominale `deepseek-flash`, sans réflexion. Révision effective des poids distants non vérifiable. |
